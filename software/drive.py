# coding: utf-8
# drive class
# Uploads images to a Google Drive account in a hierarchical structures year/month/day inside de detected/ folder
...
import sys, time, os.path, atom.data, gdata.client, gdata.docs.client, gdata.docs.data, mimetypes
...
class drive:
    ...
    def __init__(self, username, password):
        ...
        self.client   = gdata.docs.client.DocsClient(source='people-detection-turret');
    ...
    def get_path(self, time):
        ...
        try:
            detected_folder = self.client.GetResources(q=q).entry[0]
        # If detected/ is not found, create it and add the current year, month and day to the time hierarchy
            ...
            detected_folder = self.client.CreateResource(detected_folder);
            year_folder = gdata.docs.data.Resource(type='folder', title=str(time.year));
            year_folder = self.client.CreateResource(year_folder, collection=detected_folder);
            month_folder = gdata.docs.data.Resource(type='folder', title=str(time.month) + ". " + time.strftime('%B') + " " + str(time.year));
            month_folder = self.client.CreateResource(month_folder, collection=year_folder);
            day_folder = gdata.docs.data.Resource(type='folder', title=str(time.day));
            day_folder = self.client.CreateResource(day_folder, collection=month_folder);
        ...
        contents_detected = self.client.GetResources(uri=detected_folder.content.src)
        for year in contents_detected.entry:
            if year.title.text == str(time.year):
                ...
                months = self.client.GetResources(q=q2).entry[0]
                contents_year = self.client.GetResources(uri=months.content.src)
                for month in contents_year.entry:
                    if month.title.text == str(time.month) + ". " + time.strftime('%B') + " " + str(time.year):
                        ...
                        days = self.client.GetResources(q=q3).entry[0]
                        contents_month = self.client.GetAllResources(uri=days.content.src)
                        ...
                        day_folder = gdata.docs.data.Resource(type='folder', title=str(time.day));
                        day_folder = self.client.CreateResource(day_folder, collection=month);
                        return day_folder.get_resumable_create_media_link().href;
                
                # If current month folder not found, create it
                month_folder = gdata.docs.data.Resource(type='folder', title=str(time.month) + ". " + time.strftime('%B') + " " + str(time.year));
                month_folder = self.client.CreateResource(month_folder, collection=year);
                day_folder = gdata.docs.data.Resource(type='folder', title=str(time.day));
                day_folder = self.client.CreateResource(day_folder, collection=month_folder);
                return day_folder.get_resumable_create_media_link().href;
    ...
    def save_img(self, img_path, uri):
        ...
        uploader = gdata.client.ResumableUploader(self.client, img_file, file_type, file_size, chunk_size=1048576, desired_class=gdata.data.GDEntry)
